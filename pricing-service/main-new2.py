...
def _fetch_stooq_latest(stooq_symbol: str) -> dict:
    ...
    try:
        ...
        # Parse CSV line: Symbol,Date,Time,Open,High,Low,Close,Volume
        parts = text.split(',')
        if len(parts) < 7:
            raise ValueError(f"Stooq: Invalid CSV format for '{stooq_symbol}'")
        
        # Handle the case where date/time might be N/D
        date_str = parts[1].strip()
        time_str = parts[2].strip() if len(parts) > 2 else "00:00:00"
        
        if date_str == "N/D" or not date_str:
        ...
        return {
            'price': close_price,
            'open': float(parts[3]) if len(parts) > 3 else close_price,
            'high': float(parts[4]) if len(parts) > 4 else close_price,
            'low': float(parts[5]) if len(parts) > 5 else close_price,
            'volume': int(float(parts[7])) if len(parts) > 7 and parts[7].replace('.','').isdigit() else 0,
            'timestamp': timestamp,
        }
    except requests.exceptions.RequestException as e:
        raise ValueError(f"Stooq request failed: {e}")
    except (ValueError, IndexError) as e:
        raise ValueError(f"Stooq parse error for '{stooq_symbol}': {e}")
